...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        company3, created = Company.objects.get_or_create(
            name='Bluestock Innovations'
        )
        company3.logo_url = 'https://bluestock.in/static/assets/logo/logo.webp'
        company3.save()
        self.stdout.write(self.style.SUCCESS(
            f"{'Created' if created else 'Updated'} Company: {company3.name}"
        ))
        ...
        self.stdout.write(self.style.SUCCESS(
            f"{'Created' if created else 'IPO already exists for'} {ipo1.company.name} (Upcoming)"
        ))
        ...
        Document.objects.get_or_create(
            ipo=ipo1,
            defaults={
                'rhp_pdf': 'ipo_documents/rhp/tech_innovators_rhp.pdf',
                'drhp_pdf': 'ipo_documents/drhp/tech_innovators_drhp.pdf'
            }
        )
        ...
        self.stdout.write(self.style.SUCCESS(
            f"{'Created' if created else 'IPO already exists for'} {ipo2.company.name} (Ongoing)"
        ))
        ...
        self.stdout.write(self.style.SUCCESS(
            f"{'Created' if created else 'IPO already exists for'} {ipo3.company.name} (Listed)"
        ))
        ...
        self.stdout.write(self.style.SUCCESS(
            f"{'Created' if created else 'IPO already exists for'} {ipo4.company.name} (Upcoming)"
        ))
        ...
        self.stdout.write(self.style.SUCCESS(
            f"{'Created' if created else 'IPO already exists for'} {ipo5.company.name} (Upcoming)"
        ))
        ...
        faq1, created = FAQ.objects.get_or_create(
            ...
            defaults={
                'answer': 'An Initial Public Offering (IPO) is the process by which a private company can go public by offering its shares to the public.',
                'order': 1
            }
        )
        self.stdout.write(self.style.SUCCESS(
            f"{'Created' if created else 'FAQ already exists'}: {faq1.question}"
        ))

        faq2, created = FAQ.objects.get_or_create(
            question='How can I apply for an IPO?',
            defaults={
                'answer': 'You can apply for an IPO through your demat account linked with a brokerage firm via ASBA (Application Supported by Blocked Amount).',
                'order': 2
            }
        )
        self.stdout.write(self.style.SUCCESS(
            f"{'Created' if created else 'FAQ already exists'}: {faq2.question}"
        ))
